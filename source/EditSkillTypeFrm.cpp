...
void __fastcall TEditSkillTypeForm::FormCreate(TObject *Sender)
{
 ...
 Table->Cells[4][1]=FoldSkill->flags&MAGIC?"   x":"";
 Table->Cells[5][1]=FoldSkill->flags&COMBAT?"   x":"";
 Table->Cells[6][1]=FoldSkill->flags&CAST?"   x":"";
 Table->Cells[7][1]=FoldSkill->flags&FOUNDATION?"   x":"";
 ...
 if(FnewSkill){
  ...
  Table->Cells[4][2]=FnewSkill->flags&MAGIC?"   x":"";
  Table->Cells[5][2]=FnewSkill->flags&COMBAT?"   x":"";
  Table->Cells[6][2]=FnewSkill->flags&CAST?"   x":"";
  Table->Cells[7][2]=FnewSkill->flags&FOUNDATION?"   x":"";
...
void __fastcall TEditSkillTypeForm::bnOkClick(TObject *Sender)
{
 ...
 FoldSkill->flags=0;
 if(cbMagic->Checked)FoldSkill->flags|=MAGIC;
 if(cbCast->Checked)FoldSkill->flags|=CAST;
 if(cbCombat->Checked)FoldSkill->flags|=COMBAT;
 if(cbFoundation->Checked)FoldSkill->flags|=FOUNDATION;
 int ind;
