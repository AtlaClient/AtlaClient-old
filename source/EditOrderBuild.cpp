    ...
    : TForm(Owner),ord(_ord)
{
 ...
 for(int i=list->Count-1;i>=0;i--){
  ...
  buil->prod=un->GetSkill(buil->otype->skill)*un->GetMen();
  if(buil->otype->item==WoodOrStoneStr){
   buil->have=un->enditems->GetNum("WOOD")+un->enditems->GetNum("STON");
  }else
   buil->have=un->enditems->GetNum(buil->otype->item);
 }
 cbBuildingChange(0);
}
//---------------------------------------------------------------------------
